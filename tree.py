...
class Tree(object):
    ...
    def depth_first_search(self, data):
        ...
        """
        options = [root]   #putting the root into a list of possible options

        while len(options > 0):   #as long as there are options left
            if options[0] == None:  #if the first option is none, remove it
                options.remove[0]   # please remove
            elif options[0].#not sure what to put here but it has to equal something else == :  #if the first option is what we are looking for, return it
                return options[0]
            else: #otherwise, replace the first option with all of its children
...
# [ root]
#  [ rootChild1, rootChild2, rootChild3] (check all one by one)
#replace every entry by its children
#  [rootChild1Child1, rootCHild1Child2, rootChild2Child1, rootChild2Child2, None] (say, rootChild3 has no children)
